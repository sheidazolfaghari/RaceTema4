...
```{r}
#podemos ver cada uno de variable 
cran$date
cran$time
cran$size
cran$r_version
...
glimpse(cran)
#it gives us a glimpse of all the rows and columns and each variable
...
```{r}
select(cran,cran$r_arch,cran$country)
...
```{r}
cran2%>% arrange(country, ip_id, desc(r_version))
...
```{r}
cran3<- cran%>% summarise(avg_bytes = mean(size))
...
top_counts <- cran %>%
  group_by(package) %>%
  summarise(count = n()) %>%
  mutate(threshold = quantile(count, probs = 0.99)) %>%
